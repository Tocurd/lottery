...
class Loader{
	...
	static public function view($view_path = array() , $push_data = array() , $view_type = 'admin' , $print = true){
		$view = '';
		$config_list = array_merge(self::$CI->config->item('seeting') , $push_data);
		$view .= self::$CI->parser->parse('/template/header' , $config_list , $print);
		foreach ($view_path as $path) {
			$view .= self::$CI->parser->parse('/' . $path , array_merge($config_list , $push_data) , $print);
		}
		if($print){
			echo $view;
		}else{
